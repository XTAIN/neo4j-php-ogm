...
class SharedRolesRepository extends BaseRepository
{
    /**
     * @return HederaCollection $item [n => SharedRoles, scopes => HederaCollection<SharedScopes>]
     */
    public function loadRolesWithScopes()
    {
        $graph = last(explode('\\', $this->getClassName()));

        $cqlRel = 'MATCH (n:' . $graph . ')-[:SHARED_ROLES_TO_SHARED_SCOPES]->(scopes) RETURN n, scopes';
        $cqlWithoutRel = 'MATCH (n:' . $graph . ') WHERE NOT (n)-[:SHARED_ROLES_TO_SHARED_SCOPES]->() RETURN n';
